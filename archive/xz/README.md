...
This package wraps `github.com/ulikunitz/xz` behind the same `Compress` and `Decompress` shape used by the other archive packages. Use `xz.NewXz()` when you want a simple stream adapter and `CheckExt` when validating archive names.

## Local Usage

- `Compress(in, out)` writes XZ-compressed bytes to `out`.
- `Decompress(in, out)` restores the original stream.
- `cd archive && go test ./...`
- `go run ./xz/test`
