...
import (
	...
	"github.com/colt3k/utils/archive/flate"
)

func main() {

	str := "just some regular text to compress and decompress, just some regular text to compress and decompress flate"
	fmt.Println("Original: ",str)

	var byt bytes.Buffer
	flate.FLATE.Compress(strings.NewReader(str), &byt)
	...
	var b2 bytes.Buffer
	flate.FLATE.Decompress(bytes.NewReader(dec), &b2)
